...
const Services = () => {
  ...
  return (
    ...
    >
      <div className="md:mb-9 md:grid md:grid-cols-hero xl:grid-cols-2 xl:gap-1 ">
        <SectionHeading title={title} className="max-md:mb-6" />
        <p className="text-right text-[43px]/[52px]  font-extralight max-md:mb-4 md:self-center md:text-[67px]/[81px] xl:text-left xl:text-6xl-accent xl:font-extralight">
          {formattedNumber(currentSlide + 1)}/
          <span className="font-thin">{formattedNumber(trips.length)}</span>
        </p>
      </div>
      <div className="md:grid md:grid-cols-hero md:gap-5 xl:grid-cols-2 xl:gap-5">
            ...
            {trips.map((trip, idx) => (
              <li key={trip.title}>
                <button
                  type="button"
                  className={twMerge(
                    "text-nowrap relative inline-block text-left text-xl/[17px] font-extralight uppercase text-white/50 md:text-left md:text-[22px]/[18px] xl:text-[28px]/6",
                    `${
                      currentSlide === idx
                        ? "activSlideButton pl-5 font-medium text-white/100"
                        : ""
                    }`,
                    `${trip.title.length > 15 ? "w-min" : ""}`,
                  )}
